...
Formaat gebaseerd om MADR (Markdown Architectural Decision Records):
...
Het systeem wordt als één geheel gedeployed, maar intern opgesplitst in duidelijk afgebakende domeinmodules (bijv. Tracking, Planning, Notification). Deze modules communiceren niet rechtstreeks met elkaar, maar via een interne event bus.
...
Een gedistribueerde microservices-architectuur introduceert aanzienlijke complexiteit (service discovery, netwerkcommunicatie, deployment pipelines). Voor een klein team met een strikte deadline vormt dit een groot risico.
Door te kiezen voor een modulaire monoliet behouden we een eenvoudige deployment en ontwikkelomgeving, terwijl we via events toch een duidelijke scheiding tussen componenten afdwingen.

### 2. Losse koppeling via events (Maintainability & Extensibility)

Binnen de monoliet passen we een event-driven aanpak toe, waarbij modules communiceren via events in plaats van directe method calls.
Dit zorgt voor:
lagere afhankelijkheden tussen modules
eenvoudigere uitbreidbaarheid (nieuwe functionaliteit kan reageren op bestaande events)
...
Dit is cruciaal voor real-time tracking, waar snelle verwerking van GPS-updates vereist is.
...
Daarnaast kunnen events intern gebufferd en verwerkt worden, wat bijdraagt aan een robuuste verwerking van updates.

### 5. Beperkte maar voldoende schaalbaarheid (Scalability)

Hoewel we geen onafhankelijke schaalbaarheid per module hebben, kunnen we de applicatie horizontaal schalen (meerdere instanties).
Voor de initiële scope van het systeem is dit voldoende. Indien de load significant toeneemt, kan deze architectuur evolueren naar microservices, waarbij de bestaande modules als basis dienen.
...
Hoge performantie door interne communicatie (geen netwerk calls)
...
Microservices (gedistribueerde domeinpartitionering) bieden sterke voordelen op vlak van schaalbaarheid, fault isolation en flexibiliteit. In combinatie met event-driven communicatie zijn ze bijzonder geschikt voor grootschalige real-time systemen.
Echter, de operationele complexiteit (monitoring, debugging, netwerkcommunicatie en data consistentie) vormt een aanzienlijk risico voor een klein team binnen een beperkte tijdspanne. Daarom wordt deze optie beschouwd als een logische evolutie voor de toekomst, maar niet als de beste keuze voor de initiële implementatie.
...
Microsoft. (2023). Event-driven architecture style. https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/event-driven
Microsoft. (2023). Microservices architecture. https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/microservices
